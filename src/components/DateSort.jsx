...
const DateSort = () => {
  ...
  const options = [
    { value: 'schedule', label: 'Default', icon: <ContactRound className="h-4 w-4" /> },
  ...
  return (
    <div className="inline-block py-1 sm:py-2 relative">
      <div 
        className="px-2 sm:px-4 py-1 sm:py-2 border border-gray-500 rounded-sm text-xs sm:text-sm dark:bg-[#22283666] dark:text-white text-[#000000b2] focus:outline-none flex items-center gap-2 cursor-pointer"
      ...
      {isOpen && (
        <div className="absolute   mt-1 w-full z-100 bg-white dark:bg-[#222836] border border-gray-300 dark:border-gray-600 rounded-md shadow-lg">
          {options.map((option) => (
            <div
              key={option.value}
