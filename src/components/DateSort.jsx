...
const DateSort = () => {
  const { filterBasis, setFilterBasis,setIsContentExpanded,isContentExpanded } = useContext(MedContext);
  ...
  return (
    <div className="gap-4 py-1 flex sm:py-2 relative">
      <div
      ...
      {isOpen && (
        <div className="absolute right-0.5 mt-1 w-[140%] z-100 bg-white dark:bg-[#222836] border border-gray-300 dark:border-gray-600 rounded-md shadow-lg">
          {options.map((option) => (
            <div
              key={option.value}
              className={`px-4 py-2 text-xs sm:text-sm flex items-center gap-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 ${filterBasis === option.value ? 'bg-gray-200 dark:bg-gray-600' : ''
                }`}
