...
const MidHeader = () => {
  ...
  const { isUserSelected } = useContext(MedContext);
  ...
  return (
    <div className="w-full mb-2 sm:mb-3 ">
      ...
      <div className="flex gap-2 sm:gap-3 flex-wrap text-xs sm:text-sm">
        <Link 
          to="/"
          className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1 sm:py-2 ${isAppointmentsActive ? "bg-white dark:bg-black dark:text-white rounded-xs drop-shadow-lg" : "rounded-sm drop-shadow-lg text-[#22283699] dark:text-gray-300"}`}
        >
